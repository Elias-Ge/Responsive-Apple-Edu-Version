...
// npm i cors
app.use(cors(["http://localhost:3000"])); // fof react class

// console.log(decrypt(key, "123456"));

var mysqlConnection = mysql.createConnection({
    user: "apple", // replace with your username
    password: "apple", // replace with your password
    host: "localhost", // replace with localhost or 127.0.0.1
    database: "apple", // replace with your database name
});
...
app.get("/iphones", (req, res) => {
    mysqlConnection.query(
        ...
        (err, rows) => {
            let iphones = { IphoneProducts: [] };
            iphones.IphoneProducts = rows;
            // var stringIphones = JSON.stringify(iphones);
            if (!err) res.json(iphones);
